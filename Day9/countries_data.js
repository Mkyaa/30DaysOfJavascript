...
// countries_data.sort((a, b) => a.name < b.name)
// console.log(countries_data)

// countries_data.sort((a, b) => a.population - b.population)  // küçükten büyüğe
// console.log(countries_data)

// countries_data.sort((a, b) => b.population - a.population) // büyükten küçüğe
// console.log(countries_data)

// countries_data.sort((a, b) => a.name.localeCompare(b.name))
// console.log(countries_data)

// countries_data.sort((a, b) => b.name.localeCompare(a.name))
// console.log(countries_data)
...
const mostSpokenLanguages = (countries, num) => {
    let arr = [],
        count = 0
    const langFilter = countries.map(country => country.languages).join(',').split(',').sort()
    for (i = 0; i < langFilter.length; i++) {
        const langObject = arr.filter(lang => lang.language === langFilter[i])[0]
        if (langObject === undefined) {
            arr.push({ language: langFilter[i], count: 1 })
        }
        else {
            langObject.count++
        }
...
console.log(mostSpokenLanguages(countries_data, 777))

const mostPopulatedCountries = (countries, num) => {
    let arr = []
    const nameFilter = countries.map(country => country.name)
    const popFilter = countries.map(country => country.population)
    for (i = 0; i < nameFilter.length; i++) {
        const popObject = arr.filter(n => n.name === nameFilter[i] && n.population === popFilter[i])[0]
        if (popObject === undefined) {
            arr.push({ name: nameFilter[i], population: popFilter[i] })
        }
    }
    arr.sort((a, b) => b.population - a.population)
    return arr.slice(0, num)
}
console.log(mostPopulatedCountries(countries_data, 17))
...
const statistics = {
    ...
    median(arr) {
        let medianNumOdd = (arr.length + 1) / 2,
            medianNumEven1 = arr.length / 2,
            medianNumEven2 = (arr.length / 2) + 1
        arr.sort((a, b) => a - b)
        if (arr.length % 2 !== 0) {
            return arr[medianNumOdd - 1]
        }
        else {
            return `${arr[medianNumEven1]} and ${arr[medianNumEven2]}`
        }
    },
    mode(arr) {
        let modeArr = []
        for (let i = 0; i < arr.length; i++) {
            const arrfilter = modeArr.filter(number => number.mode === arr[i])[0]
            if (arrfilter === undefined) {
                modeArr.push({ mode: arr[i], count: 1 })
            }
            else {
                arrfilter.count++
            }
        ...
        return modeArr.slice(0, 1)
    },
    var(arr) {
        let elementsMinusMean = 0, result = 0
        for (let i = 0; i < arr.length; i++) {
            elementsMinusMean += ((arr[i] - this.mean(arr)) ** 2)
        }
        result = (elementsMinusMean / arr.length)
    ...
    std(arr) {
        ...
        result = Math.sqrt((elementsMinusMean / arr.length))
...
// DAY 10 LEVEL 3 EXERCISES


const mostSpokenLanguagesAlternative = (arr, num) => {
    const langFilter = arr.map(country => country.languages).join(',').split(',').sort()
    let countrySet = new Set(langFilter)
    console.log(countrySet.size)
    const counts = [],
         count = {}
    for (const lang of countrySet) {
        const filteredLang = langFilter.filter(lng => lng === lang)
        counts.push({languages : lang, count : filteredLang.length})
    }
    counts.sort((a, b) => b.count - a.count)
    return counts.slice(0, num)
}
console.log(mostSpokenLanguagesAlternative(countries_data, 10))